...
export class ProductManagementComponent implements OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    ...
    if (!token) return;
    const res = await fetch(`${API_URL}/products/mis-productos`, {
  ...
  async onImagenSeleccionada(event: any, producto: any) {
    ...
    const res = await fetch(`${API_URL}/products/update/${producto._id}`, {
      method: 'PUT',
      headers: { 'Authorization': `Bearer ${token}` },
      body: formData
    });
  ...
  async guardarCampo(producto: any) {
    const token = localStorage.getItem('token');
    await fetch(`${API_URL}/products/update/${producto._id}`, {
      method: 'PUT',
      headers: {
